...
export default function InterchangeOpsPage() {
  ...
  const steps: CalcStep[] = [
    ...
    {
      label: "Merge Influence Area Density (representative model)",
      reference: "Simplified HCM freeway merge regression form",
      formula: "D_R = 5.475 + 0.00734·v_R + 0.0078·v12 − 0.00627·L_A",
      substitution: `v_R=${fmt(mergeResults.vR, 0)}, v12=${fmt(mergeResults.v12, 0)}, L_A=${accelLaneFt} ft`,
    ...
    <ModuleShell
      ...
      sidebar={
        <div className="flex flex-col gap-3">
          ...
          <NumberField label="PHF" value={phf} min={0.8} max={0.98} step={0.01} onChange={setPhf} />
          <NumberField label="Heavy Vehicle %" value={heavyPct} min={0} max={40} step={1} onChange={setHeavyPct} unit="%" />
          <NumberField label="PCE (E_T)" value={pce} min={1.5} max={2.5} step={0.1} onChange={setPce} />

          <SectionLabel>Merge / Diverge</SectionLabel>
          <NumberField label="Ramp Demand" value={rampDemandVph} step={10} onChange={setRampDemandVph} unit="vph" />
          <NumberField label="Freeway Upstream Volume" value={freewayUpstreamVph} step={50} onChange={setFreewayUpstreamVph} unit="vph" />
          <NumberField label="Freeway Lanes" value={freewayLanes} min={2} max={8} step={1} onChange={setFreewayLanes} />
          <NumberField label="Accel/Decel Lane Length" value={accelLaneFt} step={10} onChange={setAccelLaneFt} unit="ft" />

          <SectionLabel>Weaving Section</SectionLabel>
          <NumberField label="Weaving Volume" value={weaveVph} step={10} onChange={setWeaveVph} unit="vph" />
          <NumberField label="Non-Weaving Volume" value={nonWeaveVph} step={10} onChange={setNonWeaveVph} unit="vph" />
          <NumberField label="Weaving Segment Length" value={weaveLengthFt} step={50} onChange={setWeaveLengthFt} unit="ft" />
          <div className="grid grid-cols-2 gap-2">
            <NumberField label="Min. Weaving Lanes" value={minWeaveLanes} min={1} max={3} step={1} onChange={setMinWeaveLanes} />
            <NumberField label="Total Lanes" value={totalWeaveLanes} min={2} max={8} step={1} onChange={setTotalWeaveLanes} />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <NumberField label="Ramp FFS" value={rampFfs} min={25} max={55} step={1} onChange={setRampFfs} unit="mph" />
            <NumberField label="Mainline FFS" value={mainlineFfs} min={55} max={75} step={1} onChange={setMainlineFfs} unit="mph" />
          </div>

          <SectionLabel>Ramp Terminal Signal (CMA)</SectionLabel>
          <div className="grid grid-cols-2 gap-2">
            <NumberField label="Critical Vol. N-S" value={criticalNS} step={10} onChange={setCriticalNS} unit="vph" />
            <NumberField label="Sat. Flow N-S" value={satNS} step={10} onChange={setSatNS} unit="vphpl" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <NumberField label="Critical Vol. E-W" value={criticalEW} step={10} onChange={setCriticalEW} unit="vph" />
            <NumberField label="Sat. Flow E-W" value={satEW} step={10} onChange={setSatEW} unit="vphpl" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <NumberField label="Phases" value={numPhases} min={2} max={8} step={1} onChange={setNumPhases} />
            <NumberField label="Lost Time / Phase" value={lostTimePerPhase} min={2} max={6} step={0.5} onChange={setLostTimePerPhase} unit="s" />
          </div>

          <SectionLabel>D/D/1 Queuing</SectionLabel>
          <NumberField label="Oversaturated Duration" value={oversatDurationMin} min={1} max={120} step={1} onChange={setOversatDurationMin} unit="min" />
          <NumberField label="Post-Peak Arrival Rate" value={postPeakVph} step={10} onChange={setPostPeakVph} unit="vph" />
        </div>
      }
    >
